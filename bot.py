...
def load_status():
    with open('status5.json', 'r') as f:
        return json.load(f)

def save_status(data):
    with open('status5.json', 'w') as f:
        json.dump(data, f, indent=4)
    push_to_github()

def push_to_github():
    try:
        subprocess.run(['git', 'add', 'status5.json'], check=True)
        subprocess.run(['git', 'commit', '-m', 'Mise à jour de status5.json'], check=True)
