...
async def handle_access_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in authorized_users:
        if text == ACCESS_CODE:
            ...
            await update.message.reply_text("❌ Code incorrect. Veuillez réentrée le code d'accès Correcte :")
    else:
...
async def request_transaction_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_photo("AgACAgQAAxkBAAIzu2fecoNej_qLKFfhueascpr7FPE7AAJoxzEbAWaZUMd_k6AW8sZ-AQADAgADeQADNgQ")
                ...
                if referred_by:
                    for ref_user in status_data["scripts"]:
                        if ref_user["id"] == referred_by:
                            ref_user["affiliation_balance"] += 1000
                            break
