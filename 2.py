...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "👤 Voir mes informations":
        ...
        await verify_payment(update, context, text)
...
async def verify_payment(update: Update, context: ContextTypes.DEFAULT_TYPE, transaction_id: str):
    user_id = update.effective_user.id
    user_choices[user_id]["transaction_id"] = transaction_id

    if await verify_transaction(transaction_id):
        await update.message.reply_text("✅ Paiement vérifié avec succès !")
        await confirm_payment(update, context)
    else:
        await update.message.reply_text("❌ Transaction non trouvée. Veuillez réessayer ou contacter le service client.")

async def verify_transaction(transaction_id: str) -> bool:
    # Simuler une vérification de transaction (à adapter avec votre logique réelle)
    return True  # Remplacez par la logique de vérification réelle
...
async def confirm_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    remaining_balance = user_choices[user_id].get("remaining_balance", 0)
